...
// Menü elemanlarını render eden fonksiyon

const renderCards = (data) => {
  // Data dizisini dön ve her bir eleman için html oluştur
      ...
      <a href='/detail.html?id=${
        ...
        <div>
          <div class="d-flex justify-content-between">
            <h5>${item.title}</h5>
            <p class="text-success fw-bold">${(item.price * 25).toFixed(2)}₺</p>
          </div>
          <p class="lead">
          ${item.desc}
          </p>
...
const renderDetailPage = (product, outlet) => {
  // Bu fonksiyona verilen
  // outlet parametresinin içeriğini belirle
  outlet.innerHTML = `<div class="d-flex justify-content-between fs-6">
      ...
      <h4 class="mt-4">
        Ürünün Fiyatı:<span class="text-success">${(product.price * 25).toFixed(
          2
        )} ₺</span>
      </h4>
...
// Ürün Bulunamadağında renderlama yapacak fonksiyon
